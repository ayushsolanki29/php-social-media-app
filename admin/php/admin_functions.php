...
function grantBlueTick($user_id){
    global $db;
    $query="UPDATE users SET blue_tick=1 WHERE id=$user_id";
    return mysqli_query($db,$query);
}
function revokeBlueTick($user_id){
    global $db;
    $query="UPDATE users SET blue_tick=0 WHERE id=$user_id";
    return mysqli_query($db,$query);
}
